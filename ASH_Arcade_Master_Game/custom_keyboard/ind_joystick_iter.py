...
button_neopixels[8].fill((230,191,0))
...
while True:
    ...
    if joysticks[0].value == False:  # down
        p1_joystick_lights.extend([11, 12, 13, 14, 15])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[0])
        joystick_neopixels_states[0] = 1
        neopixel_changed = True
    else:
    ...
    if joysticks[1].value == False:  # up
        p1_joystick_lights.extend([3, 4, 5, 6, 7])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[1])
        joystick_neopixels_states[0] = 1
        neopixel_changed = True
    else:
        keyboard.release(keys_pressed[1])

    if joysticks[2].value == False:  # right
        p1_joystick_lights.extend([15, 0, 1, 2, 3])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[2])
        joystick_neopixels_states[0] = 1
        neopixel_changed = True
    else:
        keyboard.release(keys_pressed[2])
        
    if joysticks[3].value == False:  # left
        p1_joystick_lights.extend([7, 8, 9, 10, 11])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[3])
        joystick_neopixels_states[0] = 1
        neopixel_changed = True
    else:
        keyboard.release(keys_pressed[3])

    #p2 joy
    if joysticks[4].value == False:  # S
        p2_joystick_lights.extend([13, 14, 15, 0, 1])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[4])
        joystick_neopixels_states[1] = 1
        neopixel_changed = True
    else:
        keyboard.release(keys_pressed[4])

    if joysticks[5].value == False:  # D
        p2_joystick_lights.extend([1, 2, 3, 4, 5])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[5])
        joystick_neopixels_states[1] = 1
        neopixel_changed = True
    else:
        keyboard.release(keys_pressed[5])

    if joysticks[6].value == False:  # W
        p2_joystick_lights.extend([5, 6, 7, 8, 9])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[6])
        joystick_neopixels_states[1] = 1
        neopixel_changed = True
    else:
        keyboard.release(keys_pressed[6])

    
    if joysticks[7].value == False:  # A
        p2_joystick_lights.extend([9, 10, 11, 12, 13])
        if len(keycode_queue) < MAX_KEY_PRESSES:
            keycode_queue.append(keys_pressed[7])
        joystick_neopixels_states[1] = 1
        neopixel_changed = True
    else:
        keyboard.release(keys_pressed[7])
    
    #print("before" + str(p2_joystick_lights))
    #print(joystick_neopixels_states)
    if joystick_neopixels_states[0] == 0:
        p1_joystick_lights = []
        joystick_neopixels_states[1] = 1
        neopixel_changed = True
    if joystick_neopixels_states[1] == 0:
        p2_joystick_lights = []
        joystick_neopixels_states[1] = 1
        neopixel_changed = True
        #print("changed it!")
    ...
    if keycode_queue:
        ...
        #last_key_input = monotonic() Think this bug is fixed? 
        #clean duplicates from the queue
        cleaned_keycode_queue = list(set(keycode_queue))
        #print("clean list: " + str(cleaned_keycode_queue))
        for i in range(len(cleaned_keycode_queue)):
            ...
            for i in range(8,len(keys_pressed)):
                if keycode == keys_pressed[i]: #button was pressed 
                    #allow both black buttons to be constantly held...
                    if i != 8 or i != 12:
    ...
    '''else:
        ...
        # I THINK* this bug was fixed in windows power settings...
    ...
    if neopixel_changed:
        ...
        for i, neopixel_strip in enumerate(joystick_neopixels):
            ...
            if joystick_neopixels_states[i] == 1: #strip was updated since last call
                ...
                if i == 0:
                    #print(" light up" + str(p1_joystick_lights))
    ...
    #print("this code took: " + str(monotonic() - now))
    #sleep(.1)
